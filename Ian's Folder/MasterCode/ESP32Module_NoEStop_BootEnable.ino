...
//  UPDATED: BOOT(GPIO0) + EN(reset) diagnostics integrated.
//  E-stop removed entirely for consistent serial input.
//
//  Jetson <-> ESP32 wiring:
...
//   - GPIO0 (BOOT) is monitored for button press/release messages.
//   - ESP32 EN(reset) cannot be read directly in firmware; pressing EN resets
//     the chip, so setup() runs again and bootCount increments.
//
...
//   (Startup only / after quit) h       -> run homing routine
//   (Startup only / after quit) toggle  -> toggle motor driver enable state
// ============================================================
...
const int PUL_PIN = 4;
const int DIR_PIN = 5;
const int EN_PIN  = 6;   // TB6600 / DRV8825 enable pin (NOT ESP32 EN reset)
...
const float INDEX_TO_STOP_DEG = 31.5f;
...
const float INVENTORY_THRESH_CM = 12.5f;
...
void debugIndexEvent(const __FlashStringHelper *msg, long stepsMoved) {
  hostPrint(F("[INDEX] "));
  hostPrint(msg);
  hostPrint(F(" | stepsMoved="));
  hostPrintln(stepsMoved);
}

void printIndexDebug(const __FlashStringHelper *prefix = F("[INDEX DEBUG]")) {
  int raw = digitalRead(INDEX_PIN);
  hostPrint(prefix);
  hostPrint(F(" raw="));
  hostPrint(raw);
  hostPrint(F(" interpreted="));
  hostPrintln(raw == INDEX_ACTIVE_LEVEL ? F("PRESSED") : F("RELEASED"));
}
...
bool seekNextPressEventCCW(long &stepsMoved) {
  ...
  stepsMoved = 0;
  long guard = 0;
  bool lastRawPressed = indexPressedRaw();

  if (lastRawPressed) {
    debugIndexEvent(F("SWITCH ALREADY PRESSED at seek start"), stepsMoved);
  }

  // Wait for release if already pressed
  while (!confirmReleased()) {
    if (indexPressedRaw() && !lastRawPressed) {
      debugIndexEvent(F("RAW PRESS seen while waiting for release"), stepsMoved);
    }
    lastRawPressed = indexPressedRaw();
  ...
  debugIndexEvent(F("RELEASE CONFIRMED"), stepsMoved);
  ...
  while (true) {
    ...
    if (rawNow && !lastRawPressed) {
      debugIndexEvent(F("RAW PRESS DETECTED"), stepsMoved);
    }
    if (!rawNow && lastRawPressed) {
      debugIndexEvent(F("RAW RELEASE DETECTED"), stepsMoved);
    }
    ...
    if (confirmPressed()) {
      debugIndexEvent(F("PRESS CONFIRMED"), stepsMoved);
      ...
      while (!confirmReleased()) {
        if (indexPressedRaw() && !lastRawPressed) {
          debugIndexEvent(F("RAW PRESS during post-hit release wait"), stepsMoved);
        }
        lastRawPressed = indexPressedRaw();
      ...
      debugIndexEvent(F("POST-HIT RELEASE CONFIRMED"), stepsMoved);
      enableDriver(false);
      return true;
    }
  }
...
bool homeRoutine() {
  ...
  int raw = digitalRead(INDEX_PIN);
  hostPrint(F("INDEX digitalRead = "));
  hostPrintln(raw);
  hostPrint(F("INDEX interpreted = "));
  hostPrintln(raw == INDEX_ACTIVE_LEVEL ? F("PRESSED") : F("RELEASED"));
  ...
  for (int i = 0; i < HOME_SAMPLE_EVENTS; i++) {
    long g = 0;
    hostPrint(F("[HOME] Sampling gap "));
    hostPrint(i + 1);
    hostPrint(F("/"));
    hostPrintln(HOME_SAMPLE_EVENTS);
  ...
  hostPrint(F("Estimated normal gap = "));
  hostPrintln(normalGap);
  hostPrint(F("Homing threshold (short) = "));
  hostPrintln(encoderGapThresholdSteps);
  ...
  while (true) {
    ...
    hostPrint(F("[HOME] Scan gap = "));
    hostPrintln(g);

    if (g < encoderGapThresholdSteps) {
      hostPrint(F("Detected SHORT gap = "));
      hostPrintln(g);
...
void startupPromptAndHome() {
  ...
  String bootLine = "";
  unsigned long lastIndexDebugMs = 0;

  while (true) {
    updateBeamDebounce();
    processBootButtonMessages();

    if (millis() - lastIndexDebugMs >= 500) {
      printIndexDebug(F("[STARTUP INDEX]"));
      lastIndexDebugMs = millis();
    }
...
void printHelp() {
  ...
  if (homed) {
    hostPrint(F("Status: HOMED, currentBin=BIN"));
...
void setup() {
  ...
  printIndexDebug(F("[BOOT INDEX AFTER INPUT_PULLUP]"));
...
void loop() {
  ...
  if (line.equalsIgnoreCase("toggle")) {
    ...
    hostPrintln(F("Type 'quit' to return to startup and toggle motor driver."));
    line = "";
    printEnterCommand();
    return;
  }
