...
// ESP32-S3 default UART1 pins are often GPIO17 (TX) and GPIO18 (RX).
...
inline void hostPrint(const __FlashStringHelper *s) {
  Jetson.print(s);
  Serial.print(s);
}
inline void hostPrint(const String &s) {
  Jetson.print(s);
  Serial.print(s);
}
inline void hostPrint(const char *s) {
  Jetson.print(s);
  Serial.print(s);
}
inline void hostPrint(long v) {
  Jetson.print(v);
  Serial.print(v);
}
inline void hostPrint(int v) {
  Jetson.print(v);
  Serial.print(v);
}
inline void hostPrint(float v, int digits = 2) {
  Jetson.print(v, digits);
  Serial.print(v, digits);
}

inline void hostPrintln() {
  Jetson.println();
  Serial.println();
}
inline void hostPrintln(const __FlashStringHelper *s) {
  Jetson.println(s);
  Serial.println(s);
}
inline void hostPrintln(const String &s) {
  Jetson.println(s);
  Serial.println(s);
}
inline void hostPrintln(const char *s) {
  Jetson.println(s);
  Serial.println(s);
}
inline void hostPrintln(long v) {
  Jetson.println(v);
  Serial.println(v);
}
inline void hostPrintln(int v) {
  Jetson.println(v);
  Serial.println(v);
}
...
inline void pollEstop() {
  while (Jetson.available() > 0) {
    ...
    // Optional: don't push chars into the command line here; loop() handles line-building.
  }
...
void printHelp() {
  ...
  hostPrintln(F("Commands: h, i, bin0, bin1, bin2, bin3, e"));
  hostPrintln(F("  h        -> homing routine (find encoder double-press, set BIN0)"));
...
void loop() {
  ...
  if (readLineFromHosts(line)) {
    ...
    if (line.equalsIgnoreCase("h")) {
      hostPrintln(F("Homing... (type 'e' to stop)"));
      if (!homeRoutine()) hostPrintln(F("ERROR: homing failed."));
      line = "";
      printEnterCommand();
      return;
    }
    ...
    hostPrintln(F("Unknown command. Use 'h', 'i', 'bin0..bin3', or 'e'."));
    line = "";
    printEnterCommand();
  }
}
