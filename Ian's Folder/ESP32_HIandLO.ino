...
// ============================================================
// Output logic:
//   - Print "HI" if distance < threshold  (closer object => "has inventory")
//   - Print "LO" if distance >= threshold (farther => "low/empty")
// Change this to tune your depth threshold.
...
inline void enableDriver(bool on) {
  // break-beam logic will NOT touch ENA; motor routines still use this.
...
// ============================================================
// Rotate 2 bins so the LAST SELECTED bin is under ultrasonic.
// Then read robust distance and clear inventoryPending.
// ============================================================
...
bool runInventorySequence() {
  ...
  // ---- NEW: primary HI/LO output, with distance on its own line in parentheses ----
  if (isnan(d)) {
    Serial.println(F("LO")); // fail-safe: treat invalid as LO
    Serial.println(F("(Distance(cm) = NAN)"));
  } else {
    // Define HI as "closer than threshold" (object present / inventory present)
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      line.trim();
      if (line.length() == 0) { Serial.println(F("Enter command:")); return; }
      ...
      if (line.equalsIgnoreCase("i")) {
        if (!inventoryPending) {
          Serial.println(F("Nothing to inventory right now."));
        } else if (beamStableBlocked) {
          Serial.println(F("Gate is BLOCKED. Push bin fully in first."));
        } else if (gateLockout) {
          Serial.println(F("Gate re-arming. Wait for READY message."));
        } else {
      ...
      if (isBinCmd) {
        if (gateLockout) {
          Serial.println(F("GATE LOCKOUT: bin is out (beam blocked) or waiting 2s after reinsertion."));
          Serial.println(F("Push bin back in (beam OPEN). Then wait 2 seconds."));
          line = "";
          Serial.println(F("Enter command:"));
          return;
        }
        if (inventoryPending) {
          Serial.println(F("INVENTORY REQUIRED: Press 'i' to measure before selecting another bin."));
          line = "";
          Serial.println(F("Enter command:"));
          return;
        }
